...
def ValidateConfig(path):
    ...
    for i, task in enumerate(config):
        ...
        if not 'id' in task.keys():
            ...
            if type(task['id']) != str:
                print(
                    f"value of key id shoud be a str, not {task['id']} with type {type(task['id'])}")
                Count += 1
        ...
        if not 'start' in task.keys():
            ...
            if type(task['start']) != bool:
                print(
                    f"value of key start shoud be a bool, not {task['start']} with type {type(task['start'])}")
                Count += 1
        ...
        if not 'nextTasks' in task.keys():
            ...
            if type(task['nextTasks']) != list:
                print(
                    f"value of key nextTasks shoud be a list, not {task['nextTasks']} with type {type(task['nextTasks'])}")
                Count += 1
            else:
                for j, nextTask in enumerate(task['nextTasks']):
                    if type(nextTask) != dict:
                        print(
                            f"nextTasks[{j}]: value in array nextTasks shoud be a dict, not {nextTask} with type {type(nextTask)}")
                        Count += 1
                    else:
                        if not 'operate' in nextTask.keys():
                            print(
                                f'nextTasks[{j}]: missing key value "operate"')
                            Count += 1
                            if not nextTask['operate'] in ['start', 'stop']:
                                print(
                                    f"nextTasks[{j}]: Unknown operate {nextTask['operate']}")
                                Count += 1
                        ...
                        if not 'id' in nextTask.keys():
                            ...
                            if not nextTask['id'] in TaskId:
                                print(
                                    f"nextTasks[{j}]: Unknown Id {nextTask['id']}")
                                Count += 1
        ...
        if task['type'] == 'keypress':
            if not 'keys' in task.keys():
                ...
                if type(task['keys']) != list:
                    print(
                        f"value of key keys shoud be a list, not {task['keys']} with type {type(task['keys'])}")
                    Count += 1
                else:
                    for j, keyset in enumerate(task['keys']):
                        if type(keyset) != list:
                            print(
                                f"keys[{j}]: value in array keys shoud be a list, not {keyset} with type {type(keyset)}")
                            Count += 1
                        else:
        ...
        if task['type'] == 'match':
            ...
            if not 'bodyPart' in task.keys():
                ...
                if type(task['bodyPart']) != list:
                    print(
                        f"value of key bodyPart shoud be a list, not {task['bodyPart']} with type {type(task['bodyPart'])}")
                    Count += 1
                else:
                    f1 = True
                    for j, parts in enumerate(task['bodyPart']):
                        if type(parts) != list:
                            print(
                                f"bodyPart[{j}]: value in array bodyPart shoud be a list, not {parts} with type {type(parts)}")
                            Count += 1
                        else:
                            for part in parts:
                                if not part in ["face", "leftHand", "rightHand", "body"]:
                                    print(
                                        f"bodypart[{j}]: Unknown part {part}")
                                    Count += 1
            ...
            if not 'poseFile' in task.keys():
                ...
                if type(task['poseFile']) != list:
                    print(
                        f"value of key poseFile shoud be a list, not {task['poseFile']} with type {type(task['poseFile'])}")
                    Count += 1
                else:
                    f2 = True
                    for j, file in enumerate(task['poseFile']):
                        if type(file) != str:
                            print(
                                f"poseFile[{j}]: value in array poseFile shoud be a str, not {file} with type {type(file)}")
                            Count += 1
                        try:
            ...
            if not 'sensetive' in task.keys():
                ...
                if type(task['sensetive']) != list:
                    print(
                        f"value of key sensetive shoud be a list, not {task['sensetive']} with type {type(task['sensetive'])}")
                    Count += 1
                else:
                    f3 = True
            ...
            if not 'frames' in task.keys():
                ...
                if type(task['frames']) != list:
                    print(
                        f"value of key frames shoud be a list, not {task['frames']} with type {type(task['frames'])}")
                    Count += 1
                else:
                    f4 = True
                    for j, num in enumerate(task['frames']):
                        if type(num) != int:
                            print(
                                f"frames[{j}]: value in array frames shoud be a int, not {num} with type {type(num)}")
                            Count += 1
        ...
        if task['type'] == 'detect':
            ...
            if not 'frames' in task.keys():
                ...
                if type(task['frames']) != int:
                    print(
                        f"value of key frames shoud be a int, not {task['frames']} with type {type(task['frames'])}")
                    Count += 1
        ...
        if task['type'] == 'timeout':
            if not 'timeout' in task.keys():
                ...
                if type(task['timeout']) != int:
                    print(
                        f"value of key timeout shoud be a int, not {task['timeout']} with type {type(task['timeout'])}")
                    Count += 1
    print("="*30)
