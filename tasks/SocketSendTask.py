...
class SocketSendTask(Task):
    ...
    def validate(task: dict,ids:list,sameIds:list):
        ...
        if not 'ip' in task.keys():
            ...
            print(f"Type Error: 'ip' expects a string, but found a {type(task['ip'])}({task['ip']}) instead")
            errorCount += 1
        ...
        if not 'port' in task.keys():
            ...
            print(f"Type Error: 'port' expects an int between 0 and 65535, but found a {type(task['timeout'])}({task['timeout']}) instead")
            errorCount += 1
        
        return errorCount, warningCount

    def __init__(self, controller: object, id: str, ip: str, port: int, nextTasks: list, start: bool = True):
    ...
    def send(self, msg, x):
        ...
        while start < len(msg):
            try:
                ...
                print(
                    f"[socket] Can't send message to {self.ip}:{self.port}", file=sys.stderr)
