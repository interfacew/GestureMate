...
class TaskController:
    ...
    def readConfig(self,path: str):
        ...
        for task in config:
            taskType=task['type']
            if taskType=="detect":
                taskObject=DetectTask(self,task['id'],task['bodyPart'],task['nextTasks'],task['start'],task['command'])
            elif taskType=="match":
                taskObject=MatchTask(self,task['id'],task['bodyPart'],task['poseFile'],task['sensetive'],task['nextTasks'],task['start'],task['command'])
            elif taskType=="timeout":
                taskObject=TimeoutTask(self,task['id'],task['timeout'],task['loop'],task['nextTasks'],task['start'],task['command'])
