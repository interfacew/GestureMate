        ...
        while camera.isOpened():
            ...
            if ret:
                ...
                if not res == {}:
                    ...
                    rightHandDelta = MatchTask.calcDelta(['rightHand'], now,
                                                         res)
                    faceDelta = MatchTask.calcDelta(['face'], now, res)
                    cv.putText(image, f"body delta       : {bodyDelta:.5f}",
                               (0, 75), cv.FONT_HERSHEY_SIMPLEX, 1,
                               (0, 0, 255), 2)
                    cv.putText(image,
                               f"left hand delta  : {leftHandDelta:.5f}",
                               (0, 100), cv.FONT_HERSHEY_SIMPLEX, 1,
                               (0, 0, 255), 2)
                    cv.putText(image,
                               f"right hand delta : {rightHandDelta:.5f}",
                               (0, 125), cv.FONT_HERSHEY_SIMPLEX, 1,
                               (0, 0, 255), 2)
                    cv.putText(image, f"face delta       : {faceDelta:.5f}",
                               (0, 150), cv.FONT_HERSHEY_SIMPLEX, 1,
                               (0, 0, 255), 2)
